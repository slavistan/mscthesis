\chapter{3-Fold Compressed Sparse Row Matrix Storage Format}
  ...
  \section{Matrix-Vector Multiplication Schemes} \label{subsec:matrix-vector-multiplication-schemes}
    ...
    \subsection{Vectorized SIMD Multiplication Scheme} \label{subsubsec:vectorized-simd-multiplication-scheme}
      ...
      further. These expected benefits for such configurations are verified in a later chapter (see \Todo{Add reference
      to mvm}).
      ...
      \subsubsection{Matrix Slice Compositions and corresponding SIMD Schemes}
        ...
        \paragraph{SIMD Scheme I: Arbitrary Values \& Diagonal Structure}
          ...
          \begin{figure}[ht]
            ...
            \toccaption{Data layout and vectorized multiplication scheme of matrix slice with arbitrary values and
              diagonal structure.}{Pointer-index values are indicated by arrows to the elements they point.
              \Todo{Bildbeschreibung erzeugen und Layout der 3 Teile segmentieren}}
            \label{fig:simd-scheme-i-data-layout.tex}
          \end{figure}
        ...
        \paragraph{SIMD Scheme II: Collated Values \& Diagonal Structure}
          ...
          \begin{figure}[ht]
            ...
            \toccaption{Data layout and vectorized multiplication scheme of matrix slice with collated values and
              diagonal structure.}{Pointer-index values are indicated by arrows to the elements they point.
              \Todo{Bildbeschreibung erzeugen und Layout der 3 Teile segmentieren}}
            \label{fig:simd-scheme-ii-data-layout.tex}
          \end{figure}
        ...
        \paragraph{SIMD Scheme III: Arbitrary Values \& Uniform Structure}
          ...
          \begin{figure}[ht]
            ...
            \toccaption{Data layout and vectorized multiplication scheme of matrix slice with arbitrary values and
              uniform structure.}{Pointer-index values are indicated by arrows to the elements they point.
              \Todo{Bildbeschreibung erzeugen und Layout der 3 Teile segmentieren}}
            \label{fig:simd-scheme-iii-data-layout.tex}
          \end{figure}

        \paragraph{SIMD Scheme IV: Collated Value \& Diagonal Structure}

          Singuläre Matrizen -> Seltener genutzt. Zerfällt in skalare Multplikation
      ...
      \begin{figure}[ht]
        \centering
        \captionsetup{width=0.9\columnwidth}
        \input{./assets/mvm-simd-scheme-arbdiag-schematic.tex}
        \toccaption{Matrix slice with diagonal structure, corresponding data layout and vectorized matrix-vector
          multiplication scheme.}{A diagonal's elements are located within V at a fixed stride equal to the number of
          diagonals.  Depending on the hardware capabilities this may make the gather-load operation more performant.}
