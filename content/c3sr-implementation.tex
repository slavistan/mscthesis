...
\chapter{Implementation Details}
  ...
  \section{Run-Length-Increment Encoding}
    ...
    \section{Vectorization of Matrix-Vector Multiplication} \label{sec:vectorization-of-mvm}
      ...
      performing vectorized matrix-vector multiplication. The data array will be reffered to as COMP, the slice
      composition array.


      of a constant size corresponding to
      the hardware's vector register size has many benefits which arise from


      In order to utilize the vectorized matrix-vector multiplication schemes introduced in Section
       an analysis
