...
\chapter{Implementation Details}
  ...
  \section{Run-Length-Increment Encoding}
    ...
    \begin{figure}[ht]
      ...
      \toccaption{Run-length-increment encoding scheme.}{}
    ...
    referred to as the triplet's delta. In contrast to what the name might suggest, instead of saving the length of a
    run its first entry's index is stored. This causes the triplet indices to be sorted in ascending fashion which
    allows to implement a transparent pseudo-random access into the data structure. To access an element correponding to
    a specific index in the original, uncompressed array a simple binary search over the triplet indices is required to
    retrieve the corresponding triplet and adding the triplet's delta times the index's remaining offset to the
    triplet's initial value will produce the result.

    Random-access would be very costly, however is seldomly needed. Incremental accesses for mvm

    Struct of arrays  ..


    Note that this reuses the original premises made for the 
