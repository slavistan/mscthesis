...
\chapter{Characterization \& Benchmarks}
  ...
  \section{Matrix-Vector Multiplication with Fully Structured Matrices} \label{sec:mvm-structured}
    ...
    shown in Figure \ref{assets/test-matrices-object-size.png}.
    ...
    operations' durations which can be stated in terms of bandwidths via $t_i = d_i / b_i$. Note that the machines'
    bandwidths used for the upper limit refer to the type of memory required to hold the objects' memory for the largest
    problem types based on grids with edge sizes of $d = 200$ nodes. For the Skylake machines this corresponds to main
    memory while the KNL's high-bandwidth memory is large enough to contain all data. Figure \ref{fig:mvm-perturbed-new}
    displays the obtained benchmark results for both families of matrices derived from structured grids.
    ...
    \Todo{Plot mit TBB Implementierung zeigen}
    \Todo{Plot mit TBB hat schnelle Cacheeffekte}
